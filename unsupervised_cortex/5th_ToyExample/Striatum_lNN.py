...
class Striatum_lNN(nn.Module):
    ...
    def __init__(self, input_s, IT_inpt_s, ln_rate, h_size, device):
        ...
        self.l1 = nn.Linear(h_size*2, h_size)
    ...
    def forward(self, thalamic_input, IT_inpt):
        ...
        action_p = torch.sigmoid(self.l_rwd_output(x))
        ...
        return  action.to(torch.int64).squeeze() # convert action to int (i.e., lick or no lick)

    def update(self, rwd):
        """ update the newtork based on observed rwd"""

        loss = torch.sum(- self.p_action.squeeze() * rwd)
